 ...
 *
 * An array A of points in a 2D plane is given. These points represent a polygon: every two consecutive points describe an
 * edge of the polygon, and there is an edge connecting the last point and the first point in the array.
 *
 * A set of points in a 2D plane, whose boundary is a straight line, is called a semiplane. More precisely, any set of the
 * form {(x, y) : ax + by ≥ c} is a semiplane. The semiplane contains its boundary.
 *
 ...
 *
 *
 * If a polygon is concave (that is, it is not convex), it has a vertex which does not lie on its convex hull border.
 * Your assignment is to find such a vertex.
 *
 ...
 * function solution(A);
 *
 * that, given a non-empty array A consisting of N elements describing a polygon, returns −1 if the polygon is convex. Otherwise,
 * the function should return the index of any point that doesn't belong to the convex hull border. Note that consecutive edges of
